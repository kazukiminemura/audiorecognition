...
from src.settings import LFM2_AUDIO_REPO, MINUTES_MAX_INPUT_CHARS
from src.engine.ui_engine import RunConfig, SpeechEngine
...
class MinutesWorker(QtCore.QObject):
    ...
    def run(self):
        ...
        if not raw:
            ...
            return
        print(
            f"[minutes] worker start chars={len(raw)} out_dir={self._out_dir}",
            flush=True,
        )
        if len(raw) > MINUTES_MAX_INPUT_CHARS:
            raw = raw[:MINUTES_MAX_INPUT_CHARS]

        try:
            print(
                f"[minutes] mode=llm chars={len(raw)} max_chars={MINUTES_MAX_INPUT_CHARS}",
                flush=True,
            )
            print("[minutes] calling minutes_summarizer", flush=True)
            minutes_text = get_minutes_summarizer().summarize(raw)
        except Exception as exc:
            self.error.emit(str(exc))
            return
        print("[minutes] worker done", flush=True)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, engine: SpeechEngine):
        ...
        self.minutes_label.setObjectName("Section")
        self.minutes_copy_btn = QtWidgets.QPushButton("コピー")
        self.minutes_copy_btn.clicked.connect(self._copy_minutes)
        self.minutes_header = QtWidgets.QWidget()
        minutes_header_layout = QtWidgets.QHBoxLayout(self.minutes_header)
        minutes_header_layout.setContentsMargins(0, 0, 0, 0)
        minutes_header_layout.addWidget(self.minutes_label)
        minutes_header_layout.addStretch(1)
        minutes_header_layout.addWidget(self.minutes_copy_btn)
        self.minutes_text = QtWidgets.QTextEdit()
        ...
        layout.addWidget(self.minutes_header)
        layout.addWidget(self.minutes_text, 1)
    ...
    def _save_script(self):
        ...
        out_dir = _get_minutes_output_dir()
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        files = [
            (
                Path(out_dir) / f"{stamp}_minutes_source_{src_label}.txt",
                short_source,
            ),
            (
                Path(out_dir) / f"{stamp}_minutes_translation_{tgt_label}.txt",
                short_trans,
            ),
        ]
    ...
    def _save_minutes_from_script(self):
        ...
        if not file_path:
            print("[minutes] no file selected", flush=True)
            return

        print(f"[minutes] selected file={file_path}", flush=True)
        try:
    ...
    def _start_minutes_generation(
        ...
        self._minutes_use_translation = use_translation
        print(
            f"[minutes] start out_dir={out_dir} stamp={stamp} suffix={suffix}",
            flush=True,
        )
    ...
    def _save_minutes_text(self, minutes_text: str, use_translation: bool):
        ...
        try:
            print(f"[minutes] writing minutes to {out_path}", flush=True)
    ...
    def _clear_minutes_thread(self):
        print("[minutes] clear minutes thread", flush=True)
        if self._minutes_thread is not None:
    ...
    def _cancel_minutes_generation(self):
        if self._minutes_thread is not None:
            print("[minutes] cancel requested", flush=True)
    ...
    def _close_minutes_dialog(self):
        dialog = self._minutes_dialog
        if dialog is None:
            return
        self._minutes_dialog = None
        print("[minutes] close progress dialog", flush=True)
        try:
            dialog.canceled.disconnect(self._cancel_minutes_generation)
    ...
    def _on_minutes_finished(self, minutes_text: str):
        self._save_minutes_text(minutes_text, self._minutes_use_translation)

    def _copy_minutes(self):
        minutes_text = self.minutes_text.toPlainText().strip()
        if not minutes_text:
            QtWidgets.QMessageBox.information(
                self, "No text", "Minutes are empty."
            )
            return
        QtWidgets.QApplication.clipboard().setText(minutes_text)
...
def _get_minutes_output_dir() -> Path:
    out_dir = _project_root() / "outputs" / "minutes"
